...
exports.getAllDislikes = async (req, res, next) =>{
    id = req.params.id;
    Dislikes.findAll({ where:{userId : id}})
    .then(dislikes =>{
        if (!dislikes) {
          console.log('Not found!');
        } else { 
...
exports.likePost = async (req, res, next) => {
    ...
    if (mode === 1) {
        const like = await Likes.findOne({where: {postId: postId, userId: userId}})
        if(like === null)
        {
            post.likes += 1;
            post.save()
...
exports.dislikePost = async (req, res, next) => {
    ...
    if (mode === 1) {
        const dislike = await Dislikes.findOne({where: {postId: postId, userId: userId}})
        if(dislike === null)
        {
            post.dislikes += 1;
            post.save()
